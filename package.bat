...
@rem Patch path to ImGui in OGRE Overlay component
cd Components/Overlay
powershell -Command "(Get-Content CMakeLists.txt) -replace '"set\(IMGUI_DIR ".*"\)"', 'set\(IMGUI_DIR "${CMAKE_CURRENT_SOURCE_DIR}/src/imgui" CACHE PATH "")' | Out-File CMakeLists.txt"
cd ../../build
